...
object Partitioning {
  ...
  // --------------------------------------------------------------------------------------
  //  Affiliate old entities need a filter according to the created partitions
  // --------------------------------------------------------------------------------------
  def affliate(noeud: Noeud, prop: Set[Int], eps: Double, order: Map[Int, Int], idList: Array[Set[Int]]): Array[Partition] = {
    ...
    //    Generate a partition for each property
    for (i <- 0 to nomberPartitions - 1) {
      if( idList.contains(Set(finalPropertiesOrdered(i))) )
        {
          var p = new Partition(Set(finalPropertiesOrdered(i)))
...
//      else
//        {
//          println("----> "+idList.toList.toString()+" != "+finalPropertiesOrdered(i))
//        }
    }
...
//  ------------------------------------------------------------------------
  ...
  def getOldPartition(files: RDD[String], eps: Double, coefficient: Boolean, order: Map[Int, Int], idList: Array[Set[Int]]): RDD[(Set[Int], Array[Noeud])] = {
    ...
    val noeudsList: RDD[(Set[Int], Array[Noeud])] = files.flatMap {
      line =>
        ...
        if (!line.equals("") && !line.equals(" +")) {
          var noeud = NodeBuilder.createOldNode(line)
          ...
          if (noeud.getProperties() != null) {
            partitionsList = Partitioning.affliate(noeud, noeud.getProperties(), eps, order, idList)
          }
        }
    ...
    return noeudsList.reduceByKey(_ ++ _)
  }
  ...
  def getFinalPartitions(initialPartition: RDD[(Set[Int], Array[Noeud])], nodeCapacity: Int, eps: Double, order: Map[Int, Int]): RDD[(Set[Int], Array[Noeud])] = {
    ...
    var finalPartitions = initialPartition.flatMap { f =>
      ...
      var result: Array[Partition] = Array()
      ...
      result = Partitioning.getSmallerPartitions(p, nodeCapacity, eps, order)


      for (i <- 0 to result.length - 1) yield {

        var completeList = (result(i).getListNoeud())

        (result(i).getId(), completeList)
      }
    }
        ...
        if( order.contains(f) )
          {
            ...
            if (orderedHashMap.get(x) == None) {
              ...
              orderedHashMap = orderedHashMap.+(x -> list.sorted)
            }
          }
        //  If the actuel property didn't exist in the old dataset, it won't be used
        else
          {
            if (orderedHashMap.get(max) == None) {
              orderedHashMap = orderedHashMap.+(max -> Array(f))
            }
            else {
              var list = orderedHashMap.get(max).get

              list = list.:+(f)

              orderedHashMap = orderedHashMap.+(max -> list.sorted)
            }
          }
